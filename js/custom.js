...
function getOneWord() {
    if (usedOneWords.length === allOneWords.length) {
        usedOneWords = [];
    }

    var newOneWord = getRandomOneWord();
    while (usedOneWords.includes(newOneWord)) {
        newOneWord = getRandomOneWord();
    }
    
    usedOneWords.push(newOneWord);
    document.getElementById("oneWord").innerHTML = newOneWord;
}

function getRandomOneWord() {
    return allOneWords[Math.floor(Math.random() * allOneWords.length)];
}
